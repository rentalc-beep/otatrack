...
// Adapté de React Bits — modifié pour déclencher onItemClick(index)
// au lieu de naviguer vers un href (l'appli change d'onglet, pas d'URL).
export const StaggeredMenu = ({
  position = "right",
  colors = ["#3a2a1e", "#E8431E"],
  items = [],
  socialItems = [],
  ...
  const animateText = useCallback((opening) => {
    ...
    for (let i = 0; i < cycles; i++) { last = last === "Menu" ? "Fermer" : "Menu"; seq.push(last); }
    ...
    >
      ...
      <header className="staggered-menu-header" aria-label="Navigation">
        <div className="sm-logo" aria-label="Logo">
          <span className="sm-logo-text">OTA<span style={{ color: "var(--sm-accent)" }}>TRACK</span></span>
          {subtitle && <span className="sm-logo-sub">{subtitle}</span>}
        </div>
        <button
          ref={toggleBtnRef}
          className="sm-toggle"
          ...
          <ul className="sm-panel-list" role="list" data-numbering={displayItemNumbering || undefined}>
            {items && items.length ? items.map((it, idx) => (
              <li className="sm-panel-itemWrap" key={it.label + idx}>
                <a className="sm-panel-item" href={it.link || "#"} aria-label={it.ariaLabel || it.label} data-index={idx + 1} onClick={(e) => handleItem(e, idx)}>
                  <span className="sm-panel-itemLabel">{it.label}</span>
                </a>
              </li>
            )) : (
              <li className="sm-panel-itemWrap" aria-hidden="true"><span className="sm-panel-item"><span className="sm-panel-itemLabel">Aucun élément</span></span></li>
            )}
          </ul>
